<script lang="ts">
  ...
  import { addPoints, IKSystem, subtractPoints } from "../avatar/IKSystem";
  ...
  function drawBoneGraphics(avatar: Avatar) {
    ...
    function createBoneGraphicHierarchy(
      ...
      // Create bone graphic
      const boneGraphic = new BoneGraphic(bone.name, bone.boneMetadata.height);
  ...
  function showAllBones() {
    Object.values(boneGraphics).forEach((graphic) => graphic.setVisible(true));
  }

  function hideAllBones() {
    Object.values(boneGraphics).forEach((graphic) => graphic.setVisible(false));
  }
