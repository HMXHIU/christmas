...
<script lang="ts">
    ...
    async function updateTime(newTime: number) {
        ...
        currentTime = timeIndex(
            Math.max(0, Math.min(newTime, animation.duration)),
        );
    ...
    function keyframesByTime(
        ...
        const byTime = Object.fromEntries(
            Object.entries(groupBy(keyframes, "time")).map(
                ([time, keyframes]) => {
            ...
            {#each animation.bones as { boneName, keyframes }}
                <div class="flex items-center space-x-4">
                    ...
                    {#each keyframesByTime(keyframes, animation.duration) as [time, kf]}
                        ...
                        >
                            <div
                                class="flex flex-col text-xs font-mono text-center"
                            >
                                <p>
                                    {#if kf?.rotation != null}
                                        <span>R</span>
