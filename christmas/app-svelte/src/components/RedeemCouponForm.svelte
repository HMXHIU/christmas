<script lang="ts">
	...
	let redemptionQRCodeURL: string | null = $modalStore[0].meta.redemptionQRCodeURL;
	...
	<div class="card w-modal shadow-xl">
		...
		<section class="p-4">
			<p>
				...
				>. Make sure you are at the location before <span class="underline font-bold">using</span> it.
			</p>
		</section>
		...
		<footer class="modal-footer p-4 {parent.regionFooter}">
			{#if redemptionQRCodeURL}
				<button class="btn {parent.buttonNeutral}" on:click={parent.onClose}
					>Ok it's verified</button
				>{:else}
				<button class="btn {parent.buttonNeutral}" on:click={parent.onClose}
					>Save it for later</button
				>
				<button class="btn {parent.buttonPositive}" on:click={onRedeemCoupon}>Use it now</button>
			{/if}
		</footer>
	</div>
{/if}
