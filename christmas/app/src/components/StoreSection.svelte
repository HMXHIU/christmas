...
{#await fetchStoreMetadata(store.publicKey) then}
    <div class="flex flex-col">
        <!-- Store -->
        <header
            class="flex flex-row justify-between px-4 py-3 bg-surface-200-700-token"
        >
            <div class="flex flex-row space-x-3 my-auto">
                <div class="w-16 my-auto flex-none">
                    ...
                    {:else}
                        <Avatar
                            initials={store.account.name.slice(2)}
                            rounded="rounded-full"
                        />
                    {/if}
                </div>
            ...
            <div class="my-auto relative">
                ...
                <span
                    class="badge-icon absolute bg-surface-700 shadow-none -top-1 -right-1 z-10"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        height="16"
