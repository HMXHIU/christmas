...
export class AnchorClient {
  ...
  async getMintedCoupons(): Promise<[Coupon, number, number][]> {
    ...
    const couponSupplyBalance = await Promise.allSettled(
      mintedCoupons.map(async (coupon) => {
        try {
          const supply = (
            await this.connection.getTokenSupply(coupon.account.mint)
          ).value.uiAmount;

          const [_, regionMarketTokenAccountPda] =
            await this.getRegionMarketPdasFromMint(coupon.account.mint);
    ...
    return couponSupplyBalance
      ...
      .map(
        (x) => (x as PromiseFulfilledResult<[Coupon, number, number]>).value
      );
  }
  ...
  async getCoupons(region: string): Promise<Coupon[]> {
    ...
    const accountsWithBalance = tokenAccounts.filter(
      (x) =>
        (x.account.data as web3.ParsedAccountData).parsed.info.tokenAmount
          .uiAmount > 0
    );
    ...
    return await Promise.all(
      accountsWithBalance.map(async (tokenAccount) => {
        const mint = (tokenAccount.account.data as web3.ParsedAccountData)
          .parsed.info.mint;
        ...
        return {
          ...xs[0],
          tokenAccountData: tokenAccount.account.data as web3.ParsedAccountData,
        };
      })
    );
  }
